...
public class Decoder {
    ...
    public void setDecodedData() throws PreAmbleLengthException, FileNotFoundException, ParseException,
            ReceivedDataException, CodecProtocolException, CyclicRedundancyCheck {

        decodedData = new ArrayList<>();

        ReadLogs readLog = new ReadLogs();
        List<String> byteStrings = readLog.getHexCodes();

        for (int i = 0; i < byteStrings.size(); i++) {
            setHexCode(byteStrings.get(i));
            reader = new Reader();
            decodedData.add(new AvlDataCollection(reader));
        }
